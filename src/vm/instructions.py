...
class Instructions(Enum):
    ADD_F = 0x24		# 计算 res = sta[-1] + sta[-2]，参数为浮点数；res 入栈
    ADD_I = 0x20		# 计算 res = sta[-1] + sta[-2]，参数为整数；res 入栈
    ALLOC = 0x18		# 在堆上分配 size 字节的内存；得到的 addr 入栈
    AND = 0x2b		# 计算 res = sta[-1] & sta[-2]；res 入栈
    ...
    CMP_F = 0x32		# 比较浮点数 sta[-1] 和 sta[-2] 大小；sta[-1] < sta[-2] 时压入 -1, sta[-1] > sta[-2] 时压入 1, sta[-1] == sta[-2] 时压入 0。浮点数无法比较时压入 0。
    CMP_I = 0x30		# 比较有符号整数 sta[-1] 和 sta[-2] 大小；sta[-1] < sta[-2] 时压入 -1, sta[-1] > sta[-2] 时压入 1, sta[-1] == sta[-2] 时压入 0。浮点数无法比较时压入 0。
    CMP_U = 0x31		# 比较无符号整数 sta[-1] 和 sta[-2] 大小；sta[-1] < sta[-2] 时压入 -1, sta[-1] > sta[-2] 时压入 1, sta[-1] == sta[-2] 时压入 0。浮点数无法比较时压入 0。
    DIV_F = 0x27		# 计算 res = sta[-1] / sta[-2]，参数为浮点数；res 入栈
    DIV_I = 0x23		# 计算 res = sta[-1] / sta[-2]，参数为有符号整数；res 入栈
    DIV_U = 0x28		# 计算 res = sta[-1] / sta[-2]，参数为无符号整数；res 入栈
    DUP = 0x04		# 复制栈顶 slot
    ...
    MUL_F = 0x26		# 计算 res = sta[-1] * sta[-2]，参数为浮点数；res 入栈
    MUL_I = 0x22		# 计算 res = sta[-1] * sta[-2]，参数为整数；res 入栈
    NEG_F = 0x35		# 对 sta[-1] 取反
    NEG_I = 0x34		# 对 sta[-1] 取反
    NOP = 0x00		# 空指令
    NOT = 0x2e		# 计算 res = !sta[-1]；res 入栈
    OR = 0x2c		# 计算 res = sta[-1] | sta[-2]；res 入栈
    PANIC = 0xfe		# 恐慌（强行退出）
    ...
    SHL = 0x29		# 计算 res = sta[-1] << sta[-2]；res 入栈
    SHR = 0x2a		# 计算 res = sta[-1] >> sta[-2] （算术右移）；res 入栈
    SHRL = 0x38		# 计算 res = sta[-1] >>> sta[-2] （逻辑右移）；res 入栈
    STACKALLOC = 0x1a		# 在当前栈顶分配 size 个 slot，初始化为 0
    STORE_16 = 0x15		# 把 val 截断到 16 位存入 addr
    STORE_32 = 0x16		# 把 val 截断到 32 位存入 addr
    STORE_64 = 0x17		# 把 val 存入 addr
    STORE_8 = 0x14		# 把 val 截断到 8 位存入 addr
    SUB_F = 0x25		# 计算 res = sta[-1] - sta[-2]，参数为浮点数；res 入栈
    SUB_I = 0x21		# 计算 res = sta[-1] - sta[-2]，参数为整数；res 入栈
    XOR = 0x2d		# 计算 res = sta[-1] ^ sta[-2]；res 入栈
